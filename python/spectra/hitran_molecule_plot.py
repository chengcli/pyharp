"""Plotting and CLI helpers for HITRAN molecular line workflows."""
...
from .config import parse_broadening_composition
from .hitran_molecule_utils import (
    _build_band_and_config,
    compute_requested_absorption_spectrum,
    compute_requested_transmittance,
    load_requested_cia_dataset,
    selected_pressure_bars,
...
from .utils import default_hitran_dir, parse_wn_range, process_pool_context
from .spectrum import plot_absorption_spectrum, plot_attenuation_spectrum
...
def _add_common_arguments(parser: argparse.ArgumentParser, *, include_state: bool = True) -> None:
    ...
    parser.add_argument("--wn-range", type=parse_wn_range, default=(20.0, 2500.0))
...
def build_attenuation_parser(*, require_external_cia: bool = False, description: str | None = None, default_figure: Path | None = None) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=description or "Compute and plot molecular attenuation coefficient.")
    _add_common_arguments(parser)
    _add_external_cia_arguments(parser, required=require_external_cia)
    parser.add_argument("--figure", type=Path, default=default_figure or Path("output/molecule_attenuation_300K_1bar.png"))
    return parser
...
def build_transmission_parser(*, require_external_cia: bool = False, description: str | None = None, default_figure: Path | None = None) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=description or "Compute and plot molecular transmission over a fixed path length.")
    _add_common_arguments(parser)
    _add_external_cia_arguments(parser, required=require_external_cia)
    parser.add_argument("--path-length-km", type=float, default=1.0)
    parser.add_argument("--figure", type=Path, default=default_figure or Path("output/molecule_transmission_300K_1bar_1km.png"))
    return parser
...
def run_transmission(args: argparse.Namespace) -> None:
    _, transmittance, line_provider = compute_requested_transmittance(args)
...
def _plot_line_positions_panel(ax, line_list, *, xlim: tuple[float, float], min_line_strength: float = 1.0e-27) -> None:
    mask = (
...
def _plot_cross_section_panel(ax, spectrum, *, xlim: tuple[float, float], show_components: bool, component_label: str) -> None:
    if show_components:
        ...
        ax.plot(spectrum.wavenumber_cm1, _mask_nonpositive(spectrum.sigma_cia_cm2_molecule), label=component_label, linewidth=1.0)
    ax.plot(spectrum.wavenumber_cm1, _mask_nonpositive(spectrum.sigma_total_cm2_molecule), label="Total" if show_components else "Cross section", linewidth=1.4)
...
def _plot_attenuation_panel(ax, spectrum, *, xlim: tuple[float, float], show_components: bool, component_label: str) -> None:
    if show_components:
        ...
        ax.plot(spectrum.wavenumber_cm1, _mask_nonpositive(spectrum.attenuation_cia_m1), label=component_label, linewidth=1.0)
    ax.plot(spectrum.wavenumber_cm1, _mask_nonpositive(spectrum.attenuation_total_m1), label="Total" if show_components else "Attenuation", linewidth=1.4)
    if not _apply_positive_log_scale(
        ax,
        [
...
def _plot_transmittance_panel(ax, transmittance, *, xlim: tuple[float, float], show_components: bool, component_label: str) -> None:
    if show_components:
        ...
        ax.plot(transmittance.wavenumber_cm1, transmittance.transmittance_cia, label=component_label, linewidth=1.0)
    ax.plot(transmittance.wavenumber_cm1, transmittance.transmittance_total, label="Total" if show_components else "Transmittance", linewidth=1.4)
    ax.plot(
...
def _render_overview_page(fig, axes_flat, *, band, config, spectrum, transmittance, line_list, cia_dataset) -> None:
    ...
    fig.subplots_adjust(left=1.0 / 8.5, right=1.0 - 1.0 / 8.5, bottom=1.0 / 11.0, top=1.0 - 1.0 / 11.0, hspace=0.5)
    fig.text(
        1.0 / 8.5,
        1.0 - 0.45 / 11.0,
    ...
    _plot_line_positions_panel(axes_flat[0], line_list, xlim=xlim, min_line_strength=config.min_line_strength)
    _plot_cross_section_panel(axes_flat[1], spectrum, xlim=xlim, show_components=show_components, component_label=component_label)
    if show_components:
        _plot_secondary_component_panel(axes_flat[2], cia_dataset=cia_dataset, spectrum=spectrum, temperature_k=spectrum.temperature_k, xlim=xlim)
...
def compute_overview_products(args: argparse.Namespace):
    ...
    _, _, spectrum, line_provider = compute_requested_absorption_spectrum(args, line_db=line_db, cia_dataset=cia_dataset)
...
def build_molecule_overview_batch_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a combined multi-page PDF of molecule overview plots over one or more wavenumber ranges.")
    parser.add_argument("--hitran-dir", type=Path, default=default_hitran_dir())
...
def run_overview_batch(args: argparse.Namespace) -> None:
    ...
    for temperature_k, pressure_bar in state_pairs(args):
    ...
    with PdfPages(figure_path) as pdf:
        for (temperature_k, pressure_bar, species, wn_min, wn_max), result in zip(
            ...
            _render_overview_page(fig, axes[:, 0], band=band, config=config, spectrum=spectrum, transmittance=transmittance, line_list=line_list, cia_dataset=cia_dataset)
            pdf.savefig(fig)
            plt.close(fig)
            page_count += 1
            ...
            print(f"Added page {page_count}: T={temperature_k:.1f} K | p={pressure_bar:.3f} bar | {species} | {wn_min:.3f}-{wn_max:.3f} cm^-1 | secondary={component_label}")
    print(f"Combined overview figure: {figure_path}")
    print(f"Pages: {page_count}")


def build_overview_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a US-letter PDF overview of molecular lines, cross section, optional CIA, attenuation, and transmittance.")
    _add_common_arguments(parser)
    _add_external_cia_arguments(parser, required=False)
    parser.add_argument("--path-length-km", type=float, default=1.0)
...
def run_overview(args: argparse.Namespace) -> None:
    ...
    with PdfPages(figure_path) as pdf:
        for temperature_k, pressure_bar, result in zip(
            selected_temperatures(args),
            selected_pressure_bars(args),
            _parallel_overview_page_products(page_tasks),
            strict=True,
        ):
    ...
    _print_overview_summary(figure_path=figure_path, spectrum=spectrum, transmittance=transmittance, cia_dataset=cia_dataset, broadening_summary=broadening_summary)


def _compute_overview_page_task(args: argparse.Namespace):
    band, config, line_list, spectrum, transmittance, cia_dataset, line_provider = compute_overview_products(args)
...
def _parallel_overview_page_products(tasks: list[argparse.Namespace]) -> Iterator[tuple[object, object, object, object, object, object, str]]:
    ...
    try:
        ...
        yield from _parallel_overview_page_products_via_subprocess(tasks, max_workers=max_workers)


def _subprocess_worker_entry(task_pickle_path: str, result_pickle_path: str) -> None:
    with Path(task_pickle_path).open("rb") as handle:
        task = pickle.load(handle)
    result = _compute_overview_page_task(task)
    with Path(result_pickle_path).open("wb") as handle:
        pickle.dump(result, handle, protocol=pickle.HIGHEST_PROTOCOL)


def _run_task_via_subprocess(task: argparse.Namespace):
    code = (
        "from pyharp.spectra.hitran_molecule_plot import _subprocess_worker_entry; "
        "import sys; "
        "_subprocess_worker_entry(sys.argv[1], sys.argv[2])"
    )
    with tempfile.TemporaryDirectory(prefix="pyharp_molecule_plot_task_") as tmpdir:
        ...
        subprocess.run(
            [sys.executable, "-c", code, str(task_path), str(result_path)],
            check=True,
        )
...
def _parallel_overview_page_products_via_subprocess(
    tasks: list[argparse.Namespace],
    *,
    max_workers: int,
) -> Iterator[tuple[object, object, object, object, object, object, str]]:
