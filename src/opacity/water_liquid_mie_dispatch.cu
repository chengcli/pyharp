  ...
  AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "call_water_liquid_mie_cuda", [&] {
    ...
    native::gpu_chunk_kernel<8, 8>(
        ...
        [=] GPU_LAMBDA(char* const data[8], unsigned int strides[8],
          ...
          auto conc = reinterpret_cast<double*>(data[2] + strides[2]);
          auto wave = reinterpret_cast<double*>(data[3] + strides[3]);
          auto re = reinterpret_cast<double*>(data[4] + strides[4]);
          auto density = reinterpret_cast<double*>(data[5] + strides[5]);
          auto real = reinterpret_cast<double*>(data[6] + strides[6]);
          auto imag = reinterpret_cast<double*>(data[7] + strides[7]);
          auto* work_ptr = reinterpret_cast<ComplexDouble*>(work);
