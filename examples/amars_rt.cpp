...
int main(int argc, char** argv) {
  ...
  harp::AttenuatorOptions op1;
  op1.species_ids({0});
  harp::S8Fuller s8(op1);

  harp::AttenuatorOptions op2;
  op2.species_ids({1});
  harp::H2SO4Simple h2so4(op2);
  ...
  auto prop1 = s8->forward(conc, params);
  auto prop2 = h2so4->forward(conc, params);
