...
void interpolate_mixing_ratios(const std::vector<double>& dense_grid, const std::vector<double>& mixing_ratios, const std::vector<double>& thinned_grid, std::vector<double>& interpolated_ratios) {
    ...
    for (size_t i = 0; i < thinned_grid.size(); ++i) {
        interpolated_ratios[i] = interp1(thinned_grid[i], mixing_ratios.data(), dense_grid.data(), dense_grid.size());
    }
}
...
// unit = [w/(m^2 cm^-1)]
torch::Tensor bb_toa_flux(torch::Tensor wave, int ncol, double temp, double fscale) {
  ...
  for (int i = 0; i < nwave; ++i) {
    for (int j = 0; j < ncol; ++j) {
      bb_flux[i][j] = fscale * sr_sun * c1 * pow(wave[i], 3) / (exp(c2 * wave[i] / temp) - 1);
    }
  }
  return bb_flux;
}

std::vector<std::vector<double>> read_4width_array_from_file(std::string fpath){
...
std::tuple<std::vector<double>, std::vector<double>, std::vector<std::vector<double>>> regrid_ptx(int nlyr, int nspecies, 
  std::vector<double> p, std::vector<double> T, std::vector<std::vector<double>> mr) {
...
torch::Tensor calc_dz(int nlyr, torch::Tensor prop, const std::vector<double>& new_p, const std::vector<double>& new_rho, double g) {
  ...
  // ignoring the p grid/z grid last layer issue. really p is in center and z is on the border
  //we barely have any attenuration from the last layer anyway so just ignore this issue
  dz[nlyr - 1][0] *= 2*dz[nlyr - 2][0];

  return dz;
}

std::tuple<std::vector<std::vector<double>>, double, double> integrate_result(const torch::Tensor& result, const torch::Tensor& wave, int nlyr, int nwave) {
  std::vector<std::vector<double>> integrated_flux(nlyr + 1, std::vector<double>(2, 0.0));
  ...
  for (int i = 0; i < nwave; ++i) {
    for (int k = 0; k <= nlyr; ++k) {
      integrated_flux[k][0] += result[i][0][k][0].item<double>() * d_lambda;  //result(x,x,x,0) is flux_up
      integrated_flux[k][1] += result[i][0][k][1].item<double>() * d_lambda;  //1 is down
    }
    tot_flux_down_surf += result[i][0][0][1].item<double>() * d_lambda;
    tot_flux_down_toa += result[i][0][nlyr][1].item<double>() * d_lambda;
  }
  
  return std::make_tuple(integrated_flux, tot_flux_down_surf, tot_flux_down_toa);
}
...
int main(int argc, char **argv) {
  ...
  std::vector<std::vector<double>> aero_mr_p = read_4width_array_from_file("/home/cometz/pyharp/examples/aerosol_output_data.txt");
  std::vector<double> p(aero_mr_p.size());
  std::vector<double> T(aero_mr_p.size());
  std::vector<std::vector<double>> mr(nspecies, std::vector<double>(aero_mr_p.size()));
  ...
  auto [integrated_flux, tot_flux_down_surf, tot_flux_down_toa] = integrate_result(result, wave, nlyr, nwave);

  std::cout << "tot_flux_down_surf: " << tot_flux_down_surf << " W/m^2" << std::endl;
  std::cout << "tot_flux_down_toa: " << tot_flux_down_toa << " W/m^2" << std::endl;
  ...
  for (int k = 0; k < nlyr; ++k) {
    ...
    dT_ds[k] = -(1/(new_rho[k] * cp)) * (df_iplus1 - df)/dz[k][0].item<double>();
  ...
  outputFile2 << "#p_new[Pa] mr(new)(0) mr(new)(1) p_atmos[Pa] mr(atmos)(0) mr(atmos)(1)" << std::endl;
  for (int k = 0; k < nlyr; ++k) {
    outputFile2 << new_p[k] << " " << new_mr[0][k] << " " << new_mr[1][k] << " " << p[k] << " " <<  mr[0][k] << " " <<  mr[1][k] << std::endl;
  }
  for (int k = nlyr; k < 200; ++k) {
    outputFile2 << 0 << " " << 0 << " " << 0 << " " << p[k] << " " <<  mr[0][k] << " " <<  mr[1][k] << std::endl;
  }
  outputFile2.close();
}
