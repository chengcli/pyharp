...
from pyharp.spectra import utils
...
def test_process_pool_context_uses_spawn_on_macos(monkeypatch) -> None:
    ...
    monkeypatch.setattr(utils.mp, "get_context", fake_get_context)

    ctx = utils.process_pool_context()
...
def test_process_pool_context_prefers_fork_when_available(monkeypatch) -> None:
    ...
    monkeypatch.setattr(utils.mp, "get_all_start_methods", lambda: ["fork", "spawn"])
    calls = []
